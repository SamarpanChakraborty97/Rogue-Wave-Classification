...
edge_file_path = f"edge_data_local_original_node_classification_new_gat_num_examples_{num_examples}.npz"
...
class GAT_Graph(nn.Module):
    def __init__(self, num_gat_layers, gat_idim, gat_hdim, num_heads, dropout, training):
        super(GAT_Graph, self).__init__()

        self.num_gat_layers = num_gat_layers
        self.gat_idim = gat_idim
        self.gat_hdim = gat_hdim
        ...
        for i in range(self.num_gat_layers):
            if i==0 and i!=self.num_gat_layers-1:
                gat_layer = GATv2Conv(self.gat_idim, self.gat_hdim, self.num_heads)
                self.gat_layers.append(gat_layer)
            elif i==0 and i==self.num_gat_layers-1:
                gat_layer = GATv2Conv(self.gat_hdim, self.gat_hdim, 1)
                self.gat_layers.append(gat_layer)
            elif i>0 and i<self.num_gat_layers-1:
                gat_layer = GATv2Conv(self.gat_hdim * self.num_heads, self.gat_hdim, self.num_heads)
                self.gat_layers.append(gat_layer)
            else:
                gat_layer = GATv2Conv(self.gat_hdim*self.num_heads, self.gat_hdim, 1)
                self.gat_layers.append(gat_layer)

    def forward(self, x, edge_index, edge_weight):

        for k in range(self.num_gat_layers):
            if k==0:
                if self.training:
                    gat_out = self.dropout(x)
                    gat_out = self.elu(self.gat_layers[k](x, edge_index))
                else:
                    gat_out = self.elu(self.gat_layers[k](x, edge_index))
            else:
                if self.training:
                    gat_out = self.dropout(gat_out)
                    gat_out = self.elu(self.gat_layers[k](gat_out, edge_index))
                else:
                    gat_out = self.elu(self.gat_layers[k](gat_out, edge_index))
        return gat_out    
...
class SpatioTemporalGAT(torch.nn.Module):
    ...
    """
    def __init__(self, num_gat_layers, gat_hdim, gat_heads, dropout_prob, num_lstm_layers, lstm_hdim, training=True):
        ...
        self.spatial_module = GAT_Graph(self.num_gat_layers, self.lstm_hdim, self.gat_hdim, self.gat_heads, self.dropout_prob, self.training)
        # self.fc = Linear(self.lstm_hdim+self.gat_hdim, 2)
    ...
    def forward(self, x, edge_index, edge_weight):
        ...
        gat_in = lstm_out.view(-1, lstm_out.shape[2])
        # print(f"The shape of the input at the start of the spatial module is {gcn_in.shape}")

        gat_out = self.spatial_module(gat_in, edge_index, edge_weight)
        # print(f"The shape of the input at the end of the spatial module is {gcn_out.shape}")

        concatenated_features = torch.cat([gat_in, torch.mean(gat_out, dim=1, keepdim=True)], dim=1)
        # concatenated_features = torch.cat([gat_in, gat_out], dim=1)
...
file_str = f"NodeClassification_Local_Originallength_GAT_num_gat_layers_{num_gat_layers}"
...
model = SpatioTemporalGAT(num_gat_layers, gat_hdim, gat_heads, dropout_prob, num_lstm_layers, lstm_hdim, training=True)
...
model = SpatioTemporalGAT(num_gat_layers, gat_hdim, gat_heads, dropout_prob, num_lstm_layers, lstm_hdim, training=False)
